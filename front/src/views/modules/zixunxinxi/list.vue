<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('zixunxinxi','诊断') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="zhenduanjiluCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'诊断':'' }}</el-button>
                <el-button v-if="isAuth('zixunxinxi','诊断') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="zhenduanjiluCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'诊断':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('zixunxinxi','诊断') && contents.tableBtnIcon == 0" type="success" size="mini" @click="zhenduanjiluCrossAddOrUpdateHandler(scope.row,'cross','是','','')">{{ contents.tableBtnFont == 1?'诊断':'' }}</el-button>
    ...
    <zhenduanjilu-cross-add-or-update v-if="zhenduanjiluCrossAddOrUpdateFlag" :parent="this" ref="zhenduanjiluCrossaddOrUpdate"></zhenduanjilu-cross-add-or-update>
...
import zhenduanjiluCrossAddOrUpdate from "../zhenduanjilu/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      zhenduanjiluCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    zhenduanjiluCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.zhenduanjiluCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.zhenduanjiluCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.zhenduanjiluCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
